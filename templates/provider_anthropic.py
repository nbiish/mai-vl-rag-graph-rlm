#!/usr/bin/env python3
"""
Anthropic Template — Full VL-RAG-Graph-RLM Pipeline
...
def example_manual_pipeline():
    """Manual pipeline showing each pillar explicitly with code analysis context."""
    try:
    ...
    if has_vl:
        ...
        for snippet in code_snippets:
            store.add_text(snippet, metadata={"type": "code"})
    ...
    rlm = VLRAGGraphRLM(provider="anthropic", temperature=0.0)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Anthropic — Full VL-RAG-Graph-RLM Pipeline")
    ...
    if args.manual:
        ...
        print("Usage:")
        print("  python provider_anthropic.py --input document.pptx")
        print("  python provider_anthropic.py --manual")
