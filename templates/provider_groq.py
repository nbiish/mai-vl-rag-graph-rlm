#!/usr/bin/env python3
"""
Groq Template — Full VL-RAG-Graph-RLM Pipeline (Ultra-Fast Inference)
...
def example_full_pipeline(input_path: str, query: str = "What are the main topics covered?"):
    ...
    print(f"Sources: {len(result.sources)}, Time: {result.execution_time:.2f}s (Ultra-fast!)")
...
def example_manual_pipeline():
    ...
    texts = [
        ...
        "JSON is the standard data format for API responses.",
    ]
    ...
    rlm = VLRAGGraphRLM(provider="groq", temperature=0.0)
    ...
    print(f"Time: {result.execution_time:.2f}s (Ultra-fast!)")


def main():
    import argparse
    parser = argparse.ArgumentParser(description="Groq — Full VL-RAG-Graph-RLM Pipeline")
    ...
    if args.manual:
        ...
        print("Usage:")
        print("  python provider_groq.py --input document.pptx")
        print("  python provider_groq.py --manual")
