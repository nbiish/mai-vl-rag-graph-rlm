#!/usr/bin/env python3
"""
Generic Anthropic-Compatible Template — Full VL-RAG-Graph-RLM Pipeline

Demonstrates all six pillars with any Anthropic-compatible endpoint:
...
def example_full_pipeline(input_path: str, query: str = "What are the main topics covered?"):
    """Full 6-pillar pipeline using Anthropic-compatible client as the LLM backend."""
    try:
    ...
    client = AnthropicCompatibleClient(api_key=api_key, base_url=base_url, model_name=model_name)
...
def example_manual_pipeline():
    ...
    texts = [
        ...
        "Anthropic-compatible APIs use the /v1/messages endpoint specification.",
        "Custom proxies can route requests to different Claude deployments.",
    ]
    ...
    if has_vl:
        ...
        query = "How do Anthropic-compatible APIs work?"
        dense_results = store.search(query, top_k=10)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Anthropic-Compatible — Full VL-RAG-Graph-RLM Pipeline")
    ...
    if args.manual:
        ...
        print("Usage:")
        print("  python provider_anthropic_compatible.py --input document.txt")
        print("  python provider_anthropic_compatible.py --manual")
